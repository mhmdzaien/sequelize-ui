...
import { SchemaV0 } from '../localStorage/v0'
import { SchemaV1 } from '../localStorage//v1'
import v1JtdSchema from '../localStorage//v1/schema.jtd.json'
import { fromV1, toV1 } from '../localStorage//v1/translate'

export default class ServerStorageSchemaApi implements SchemaApi {
...
async function setSchemas(schemas: Schema[]): Promise<any> {
  return fetch('/api/schema', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(schemas.map(toV1)),
  })
}
...
async function parseV0Lazy(schema: unknown): Promise<Schema> {
  const [jtdSchema, fromV0] = await Promise.all([
    import('../localStorage/v0/schema.jtd.json'),
    (await import('../localStorage/v0/translate')).fromV0,
  ])
